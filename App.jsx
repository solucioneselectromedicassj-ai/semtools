...
// ─── CameraView — componente compartido ───────────────────────────────────────
// Maneja: selección de cámara (trasera/frontal/USB), torch/flash, captura
function CameraView({ captureLabel='📷 Capturar', onCapture, onStream, children }) {
  const vRef  = useRef();
  const cRef  = useRef();
  const tkRef = useRef(null); // video track activo
  ...
  const start = async (deviceId) => {
    try {
      ...
      onStream?.(stream);
      // Enumerar dispositivos tras obtener permiso
  ...
  const toggleTorch = async () => {
    ...
    try {
      ...
      setTorchOn(next);
    } catch(e) { setErr('Torch no soportado: '+e.message); }
  };
  ...
  // Exponer track para tacómetro
  CameraView._trackRef = tkRef;
  ...
  return (
    <>
      ...
      <div style={S.row}>
        ...
        {torchAvail && on && (
          <button
            style={{...S.btn('s'), flex:0.45, background:torchOn?C.yellow:C.surf2, color:torchOn?'#000':C.text}}
            onClick={toggleTorch}>
            {torchOn?'🔦 ON':'🔦 OFF'}
          </button>
        )}
        {on && onCapture && (
          <button style={{...S.btn(), flex:1}} onClick={capture}>
            {captureLabel}
          </button>
        )}
      </div>
...
// ─── Distancia ────────────────────────────────────────────────────────────────
// Modo A: Claude estima con objeto de referencia en la foto
// Modo B: tap-to-measure sobre la imagen capturada
function ToolDistancia() {
  const [mode,    setMode]  = useState('claude'); // 'claude' | 'tap'
  const [loading, setLoading]=useState(false);
  const [result,  setResult]=useState(null);
  const [captured,setCaptured]=useState(null); // base64
  // Tap mode state
  const [phase,   setPhase] = useState('ref1'); // ref1 | ref2 | target
  const [pts,     setPts]   = useState([]);     // [{x,y,px}]
  const [refMM,   setRefMM] = useState('85.6'); // mm de la referencia
    ...
    if (mode==='claude') {
      ...
      try {
        const r=await askClaude(b64,
          ...
          'Estimá las distancias entre los objetos visibles y el tamaño de cada uno.\n'+
          'Respondé en formato:\nREFERENCIA: [objeto usado como escala]\n'+
          'MEDIDAS:\n[lista de objetos con tamaño estimado en cm]\n'+
          'DISTANCIAS:\n[distancias entre objetos relevantes]\n'+
          'PRECISIÓN ESTIMADA: [±X%]');
        setResult(r);
      } catch(e){ setResult('⚠ '+e.message); }
      setLoading(false);
    } else {
      // Dibujar en tap canvas
      setTimeout(()=>{
  ...
  const handleTap = (e) => {
    ...
    if(phase==='ref1'){ setPhase('ref2'); }
      ...
      // Dibujar línea referencia
      ctx.beginPath(); ctx.moveTo(pts[0].x,pts[0].y); ctx.lineTo(px,py);
      ctx.strokeStyle=C.amber; ctx.lineWidth=3; ctx.setLineDash([8,4]); ctx.stroke();
      ctx.setLineDash([]);
      setPhase('target');
    } else {
      // Calcular
  ...
  return (
    <div style={S.wrap}>
      ...
      {mode==='tap' && captured && (
        <div style={{fontFamily:MONO,fontSize:10,color:C.amber,textAlign:'center'}}>
          {phaseLabel[phase]}
        </div>
      )}
      {mode==='tap' && !captured && (
        <div style={S.row}>
          ...
          <input style={{...S.inp,flex:1}} type="number" value={refMM} onChange={e=>setRefMM(e.target.value)} placeholder="mm"/>
      ...
      {!captured ? (
        <CameraView
          captureLabel={loading?'Analizando…':'📷 Capturar'}
          onCapture={loading?null:onCapture}/>
      ) : (
        <>
          {mode==='tap' ? (
      ...
      {result && (
        <div style={S.res}>
          <div style={{fontFamily:MONO,fontSize:9,color:C.dim,marginBottom:6}}>MEDICIÓN</div>
          <pre style={{fontFamily:MONO,fontSize:12,color:C.text,whiteSpace:'pre-wrap',margin:0,lineHeight:1.8}}>{result}</pre>
        </div>
      )}
      <div style={S.note}>
        {mode==='claude'?'Poné una tarjeta de crédito o regla en la foto. Claude usa eso como referencia.':'Tap 1→2: marcá la distancia conocida. Tap 3: el punto que querés medir.'}
      </div>
    </div>
  );
}
...
function ToolTacometro() {
  const [hz,     setHz]    = useState(16.7); // 16.7 Hz = 1000 RPM
  const [step,   setStep]  = useState(0.5);
  const [running,setRunning]=useState(false);
  const [mode,   setMode]  = useState('screen'); // 'screen' | 'torch'
  const [flash,  setFlash] = useState(false);    // para modo pantalla
  const [err,    setErr]   = useState(null);
  ...
  const startStrobe = async () => {
    ...
    itvRef.current=setInterval(()=>{
      state=!state;
      if(mode==='torch' && tkRef.current){
        tkRef.current.applyConstraints({advanced:[{torch:state}]}).catch(()=>{});
      } else {
        setFlash(state);
      }
    }, halfMs);
  ...
  const presets=[
    {label:'600',  hz:10},   {label:'1000', hz:16.7},
    {label:'1500', hz:25},   {label:'3000', hz:50},
  ];
    ...
    <div style={S.wrap}>
      ...
      <div style={S.row}>
        ...
        <div style={{flex:2,display:'flex',flexDirection:'column',gap:4}}>
          <select style={S.sel} value={step} onChange={e=>setStep(parseFloat(e.target.value))}>
      ...
      <button
        style={{...S.btn(running?'s':'p'), background:running?C.red:'', color:running?'#fff':''}}
        onClick={running?stopStrobe:startStrobe}>
        {running?'⏹ Detener estroboscopio':'▶ Iniciar estroboscopio'}
      </button>

      <div style={S.note}>
        Ajustá la frecuencia hasta que la pieza giratoria parezca congelada.<br/>
        Esa frecuencia × 60 = RPM del motor.<br/>
        Rango: 60–3000 RPM. Torch da mejor resultado en lugares con poca luz.
      </div>
    </div>
  );
}

// ─── Lector Jack (módulos analógicos) ────────────────────────────────────────
// Lee señal del jack de audio → interpreta según módulo: ThermoJack, AirJack, Voltaje
const JACK_MODULES = [
  { id:'raw',   label:'Señal cruda',   unit:'mV',   convert:v=>v },
  { id:'thermo',label:'ThermoJack',    unit:'°C',   convert:v=>v*100-40 }, // calibrar según circuito
...
function ToolJack() {
    ...
    try {
      ...
      const draw=()=>{
        ...
        for(let i=0;i<td.length;i++){
          const y=(1-td[i])*H/2;
          i===0?cx.moveTo(x,y):cx.lineTo(x,y); x+=sw;
        }
        cx.stroke();
        rafRef.current=requestAnimationFrame(draw);
      };
      rafRef.current=requestAnimationFrame(draw);
    } catch(e){ setErr('Sin acceso al jack: '+e.message+' — Conectá el módulo al jack de audio.'); }
  };

  const stop=()=>{
    cancelAnimationFrame(rafRef.current);
    stRef.current?.getTracks().forEach(t=>t.stop());
    setOn(false); setVal(null);
  };

  const resetMinMax=()=>{ setPeak(null); setMin(null); };
  ...
  return (
    <div style={S.wrap}>
      ...
      {/* Selector de módulo */}
      <select style={S.sel} value={module} onChange={e=>{setModule(e.target.value);resetMinMax();}}>
        {JACK_MODULES.map(m=><option key={m.id} value={m.id}>{m.label}</option>)}
      </select>

      {/* Display */}
      <div style={S.disp}>
        <span style={S.dval}>{fmtVal(val)}</span>
      ...
      {(peak!==null||min!==null) && (
        <div style={S.row}>
          <div style={{...S.disp,flex:1,textAlign:'center',padding:'10px 8px'}}>
            ...
            <div style={{fontFamily:MONO,fontSize:22,color:C.blue}}>{fmtVal(min)} {mod.unit}</div>
          </div>
          <div style={{...S.disp,flex:1,textAlign:'center',padding:'10px 8px'}}>
            <div style={{fontFamily:MONO,fontSize:9,color:C.dim}}>MÁX</div>
            <div style={{fontFamily:MONO,fontSize:22,color:C.red}}>{fmtVal(peak)} {mod.unit}</div>
          </div>
          <button style={{...S.btn('s'),flex:0.5,padding:'8px 4px',fontSize:11}} onClick={resetMinMax}>Reset</button>
        </div>
      )}
      ...
      <div style={S.note}>
        <strong style={{color:C.text}}>Módulos disponibles:</strong><br/>
        ThermoJack: NTC + LM358 → temperatura<br/>
        AirJack: hilo caliente → velocidad de flujo<br/>
        VoltJack: divisor resistivo → tensión<br/>
        Enchufá el módulo al jack 3.5mm y presioná Conectar.
      </div>
    </div>
  );
}
...
function ToolDecibeles() {
  const [db,setDb]=useState(null); const [peak,setPeak]=useState(null);
  const [on,setOn]=useState(false); const [err,setErr]=useState(null);
  const anlRef=useRef(); const rafRef=useRef(); const stRef=useRef();
  const start=async()=>{
    try{
      ...
      const tick=()=>{
        ...
        setDb(v.toFixed(1)); setPeak(p=>p===null||v>parseFloat(p)?v.toFixed(1):p);
  ...
  useEffect(()=>()=>{cancelAnimationFrame(rafRef.current);stRef.current?.getTracks().forEach(t=>t.stop());},[]);
  const v=parseFloat(db)||0; const pct=Math.min(v/120*100,100);
  ...
  return (
    <div style={S.wrap}>
      ...
      {peak&&<div style={{...S.disp,padding:'10px 16px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
      ...
      {err&&<div style={{color:C.red,fontFamily:MONO,fontSize:11}}>{err}</div>}
      {!on?<button style={S.btn()} onClick={start}>Activar micrófono</button>:<button style={S.btn('s')} onClick={stop}>Detener</button>}
    </div>
  );
}
...
// ─── Nivel ────────────────────────────────────────────────────────────────────
function ToolNivel() {
  const [ang,setAng]=useState({b:0,g:0}); const [on,setOn]=useState(false); const [err,setErr]=useState(null);
  const hRef=useRef(null);
  const start=async()=>{
    if(typeof DeviceOrientationEvent?.requestPermission==='function'){
      try{ const p=await DeviceOrientationEvent.requestPermission(); if(p!=='granted'){setErr('Permiso denegado');return;} }catch(e){setErr(e.message);return;}
    }
    const h=(e)=>setAng({b:e.beta||0,g:e.gamma||0}); hRef.current=h;
    window.addEventListener('deviceorientation',h); setOn(true); setErr(null);
  };
    ...
    <div style={S.wrap}>
      ...
      <div style={{background:'#060809',border:`1px solid ${C.bord}`,borderRadius:8,display:'flex',justifyContent:'center',padding:24,backgroundImage:SCAN}}>
        <div style={{position:'relative',width:200,height:200}}>
          ...
          <div style={{position:'absolute',top:'50%',left:'50%',width:30,height:30,marginTop:-15,marginLeft:-15,transform:`translate(${bx}px,${by}px)`,borderRadius:'50%',transition:'transform .08s ease-out,background .2s,box-shadow .2s',background:flat?`radial-gradient(circle at 35% 35%,${C.green}88,${C.green}44)`:`radial-gradient(circle at 35% 35%,${C.blue}88,${C.blue}44)`,border:`2px solid ${flat?C.green:C.blue}`,boxShadow:flat?`0 0 14px ${C.green}55`:'none'}}/>
        </div>
      </div>
      <div style={S.row}>
      ...
      {err&&<div style={{color:C.red,fontFamily:MONO,fontSize:11}}>{err}</div>}
      {!on?<button style={S.btn()} onClick={start}>Activar nivel</button>:<button style={S.btn('s')} onClick={stop}>Detener</button>}
...
// ─── Osciloscopio ─────────────────────────────────────────────────────────────
function ToolOscilo() {
  const cRef=useRef(); const anlRef=useRef(); const rafRef=useRef(); const stRef=useRef(); const actxRef=useRef();
  const [on,setOn]=useState(false); const [freq,setFreq]=useState(null); const [err,setErr]=useState(null);
  const start=async()=>{
    try{
      ...
      const src=actx.createMediaStreamSource(s); const anl=actx.createAnalyser(); anl.fftSize=2048; src.connect(anl); anlRef.current=anl;
      setOn(true); setErr(null);
      const td=new Float32Array(anl.fftSize); const fd=new Uint8Array(anl.frequencyBinCount);
      const draw=()=>{
        ...
        cx.stroke();
        anl.getByteFrequencyData(fd); let mi=0,mv=0;
  ...
  return (
    <div style={S.wrap}>
      ...
      {freq&&on&&<div style={S.disp}><span style={S.dval}>{freq}</span><span style={S.dunt}>Hz</span><div style={S.dlbl}>FRECUENCIA DOMINANTE</div></div>}
      {err&&<div style={{color:C.red,fontFamily:MONO,fontSize:11}}>{err}</div>}
      {!on?<button style={S.btn()} onClick={start}>Activar</button>:<button style={S.btn('s')} onClick={stop}>Detener</button>}
      <div style={S.note}>20 Hz – 20 kHz  •  Útil para tonos de alarma y señales de baja frecuencia.</div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={S.app}>
      ...
      {/* Nav — solo inicio cuando hay herramienta activa */}
      <div style={S.nav}>
